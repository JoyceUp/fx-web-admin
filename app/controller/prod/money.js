...
exports.withdraw = {
    ...
    exportWithdrawBankcard: function (req, res, next) {
        let header = {
            token: req.cookies.token
        };
        let params = req.body;

        let api = API.money.withdraw_export_bankcard;
        exportExcel(req, res, header, params, api, "银行卡出金");


    },
    exportWithdrawTt: function (req, res, next) {
        let header = {
            token: req.cookies.token
        };
        let params = req.body;

        let api = API.money.withdraw_export_tt;
        exportExcel(req, res, header, params, api, "电汇出金");
...
exports.internalTransferOfSystem = {
    ...
    account_info: function (req, res, next) {
        ...
        let {account_from, account_to, amount} = req.body;
        let params = {
    ...
    //转入转出合二为一
    outIn: function (req, res, next) {
        let header = {
            token: req.cookies.token
        };
        let {old_account, new_account, amount} = req.body;
        let params = {
            pre_account: old_account,
            account: new_account,
            amount: amount
        };
        request.put(API.money.pre_transfer_deposit_withdraw)
            .set(header)
            .query({msg: JSON.stringify(params)})
            .end(function (_req, _res) {
                if (_res.status === 200) {
