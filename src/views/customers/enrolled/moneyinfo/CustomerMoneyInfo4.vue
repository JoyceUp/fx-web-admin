    ...
    <section>
      <!--交易记录-->
        ...
        <el-table stripe :data="trade" style="width: 100%" size="small" tooltip-effect="dark">
            ...
            <el-table-column prop="open_time" label="开仓时间" show-overflow-tooltip></el-table-column>
            <el-table-column prop="type" label="交易类型"><template slot-scope="scope"> <span>{{ typeMapOptions[scope.row.type] }}</span></template></el-table-column>
            <el-table-column prop="symbol" label="交易品种"></el-table-column>
            <el-table-column prop="volume" label="交易量" :formatter="centTwoDollar"></el-table-column>
            <el-table-column prop="open_price" label="开仓价格"></el-table-column>
            <el-table-column prop="close_price" label="平仓价格" :formatter="isClosePrice"></el-table-column>
            <el-table-column prop="sl" label="S/L"></el-table-column>
            <el-table-column prop="tp" label="T/P"></el-table-column>
            <!--<el-table-column prop="fee" label="手续费"></el-table-column>-->
            <el-table-column prop="swaps" label="隔夜利息" :formatter="centTwoDollar"></el-table-column>
            <el-table-column label="交易盈亏"><template slot-scope="scope">{{keepDigits(scope.row.profit)}}</template></el-table-column>
            <el-table-column label="状态"><template slot-scope="scope"> <span>{{ orderStatusMapOptions[scope.row.order_status] }}</span></template></el-table-column>
        </el-table>
        <!--分页-->
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            isClosePrice(row,column){
                if(column) {
                    return row.order_status === 1 ? '' : row[column.property];
                }
                return row;
            },
            centTwoDollar(row,column){
                if(column) {
